...
class SRelStructure(TaskConfig):
    def init(self, name: str, model_dir: str, conf: Dict[str, str], planner: Any, **kwargs):
        super().init(name, model_dir, conf, planner, **kwargs)

        bundle_name = "pathology_srel_segmentation"
        # TODO: change to the correct repo - create my own repo
        ...
        #     download(name=bundle_name, version=bundle_version,
        #              bundle_dir=self.model_dir)

    def infer(self) -> Union[InferTask, Dict[str, InferTask]]:
        task: InferTask = lib.infers.SRelStructure(
            self.bundle_path, self.conf)
        return task

    def trainer(self) -> Optional[TrainTask]:
        task: TrainTask = lib.trainers.HovernetNuclei(
            self.bundle_path, self.conf)
        return task
